...
async def generate_script_async(prompt_type, current_info=None, next_info=None, comments=None):
    ...
    comment_part = ""
    is_seasonal = (prompt_type in ["opening", "closing"]) or (random.random() < 0.3)
    now_local = get_now_jst()
    ...
    if prompt_type == "opening":
        instruction = f"Write a program opening. Greet listeners. {time_context} Approx 100 words. Do NOT describe sound effects."
    elif prompt_type == "closing":
        instruction = f"Write a program closing. Bid farewell to the day. {time_context} Approx 100 words. Do NOT describe sound effects."
    else:
        ...
        if comments:
            ...
            instruction = (
                ...
                f"Then, summarize the essence of one listener's message and offer a warm, thoughtful response. "
                f"Finally, introduce {n_text}.\n"
                f"CRITICAL: Use ONLY English. NO non-English characters.\n\n"
                f"[LOG SECTION]\n"
                f"Provide a brief Japanese translation prefixed with '[LOG]'.\n\n"
                f"Messages from Unpurified Souls:\n{comments if comments else 'None'}"
            )
        else:
            instruction = f"Briefly reflect on {c_text}. {time_context} Then provide a sophisticated introduction for {n_text}. Approx 200 words. Do NOT include sound effects."
...
async def prepare_next_talk(prompt_type, current_info, next_info, comments, output_file):
    ...
    speech_text = re.sub(r'\[.*?\]', '', parts[0].strip())
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                print(f"[Error] Failed to play {current_info['title']}: {e}")

            print(f"\n♪ Now Playing: {current_info['title']}")
        ...
        if os.path.exists(final_audio):
            ...
            channel = closing_voice.play(fade_ms=200)
            while channel.get_busy(): await asyncio.sleep(0.1)
